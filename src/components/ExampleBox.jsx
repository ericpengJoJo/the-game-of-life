...
function ExampleBox() {
  ...
  return (
    ...
    >
      <ContainerWrapper width={240} height={300}>
        <div onClick={() => handleClick()} className="hover-button">
          ...
          >
            <div
              style={{
                ...
                gridTemplateRows: `repeat(${6}, ${100 / 6}%)`,
              }}
            >
              {hintDetails.length > 0 ? (
                hintDetails[hintIdx]?.pattern.map((rows, i) =>
                  rows.map((col, j) => (
                    <div
                      key={`expample-${i}-${j}`}
                      className={`rader-pixel-dot ${
                        hintDetails[hintIdx].pattern[i][j] ? "pixel-white" : ""
                      }`}
                      style={{
                        width: "100%",
                        height: "100%",
