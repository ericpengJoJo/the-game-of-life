...
function GameOfLife({ numRows, numCols }) {
  const { dispatch } = useGameContext();

  const [playBGM, { stop }] = useSound(backgroundMusic, {
    volume: 0.5,
    loop: true,
  });
  ...
  useEffect(() => {
    dispatch({
      type: "generateEmptyGameBoard",
      payload: { rows: numRows, cols: numCols },
    });
  ...
  return (
    <>
      ...
      >
        <div>
          <GameDisplay numCols={numCols} numRows={numRows} />
          <DescriptionBox />
